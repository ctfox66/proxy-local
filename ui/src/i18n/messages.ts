export type Locale = 'zh-CN' | 'en-US';

export const fallbackLocale: Locale = 'en-US';

export const messages = {
  'zh-CN': {
    app: {
      name: 'ProxyHub',
      pageTitles: {
        about: '关于',
        groups: '节点分组',
        login: '登录',
        mappings: '本地端口',
        nodes: '节点',
        settings: '设置',
      },
    },
    common: {
      add: '添加',
      addNode: '新增节点',
      addChainNode: '新增串联节点',
      addGroup: '新增分组',
      addPort: '添加端口',
      addRoute: '添加线路',
      addSubscription: '添加订阅',
      authConfigured: '有认证',
      cancel: '取消',
      close: '关闭',
      copiedEndpoint: '已复制端点',
      copyEndpoint: '复制端点',
      currentRoute: '当前',
      deleteNode: '删除节点',
      deleteGroup: '删除分组',
      deletePort: '删除端口',
      deleteSubscription: '删除订阅',
      copiedNodeUri: '已复制 URI',
      editGroup: '编辑分组',
      editNode: '编辑节点',
      editPort: '编辑端口',
      exportNodeUri: '导出 URI',
      goHome: '返回面板',
      goLogin: '去登录',
      importLinks: '导入分享链接',
      language: '语言',
      languageChinese: '简体中文',
      languageEnglish: 'English',
      languageSystem: '跟随系统',
      noAccount: '未配置账号',
      noAuth: '无认证',
      noRemark: '未填写备注',
      notSaved: '尚未保存',
      optional: '可留空',
      refreshStatus: '刷新状态',
      removeRoute: '移除线路',
      restoreDemo: '刷新数据',
      save: '保存',
      setActiveRoute: '设为当前',
      settings: '设置',
      updateBadge: '更新',
      updateTitle: '前往 {version} 的发布页',
      npmUpdateTitle: '前往 {version} 的发布页；npm 安装请执行 {command}',
      syncSubscription: '同步订阅',
      test: '测试',
    },
    home: {
      actions: {
        loadMore: '加载更多',
      },
      aria: {
        activePorts: '端口运行状态',
        addNodeMenu: '选择添加节点类型',
        closeDialog: '关闭',
        copyEndpoint: '复制端点',
        deleteNode: '删除节点',
        deletePort: '删除端口',
        disablePort: '停用端口',
        editPort: '编辑端口',
        enablePort: '启用端口',
        moreActions: '更多操作',
        removeFromPort: '从端口移除',
        removeRoute: '移除线路',
        nodeGroupFilters: '节点分组过滤',
        statusOverview: '运行概览',
        tabs: '页面切换',
      },
      dialogs: {
        addNodeLead: '名称可留空，会从 URI 自动识别。',
        addNodeTitle: '添加节点',
        addChainNodeTitle: '添加串联节点',
        addGroupLead: '创建手动分组后，可在端口线路中直接引用这个分组。',
        editGroupLead: '调整手动分组的策略和成员后，引用它的端口会随运行时刷新一起生效。',
        editGroupTitle: '编辑分组',
        importNodeLead: '可一次粘贴多条分享链接，先预览再确认导入。',
        importNodeTitle: '批量导入节点',
        editNodeTitle: '编辑节点',
        addPortTitle: '添加端口',
        editPortTitle: '编辑端口',
        mappingLead: '默认不配置用户名和密码。',
        testMappingTitle: '测试端口',
        testNodeTitle: '测试节点',
      },
      confirm: {
        deleteGroupMessage: '将删除分组「{name}」，并从端口线路和节点中移除。此操作不可撤销。',
        deleteGroupTitle: '删除分组？',
        deleteNodeMessage: '将删除节点「{name}」，并从端口线路和节点组中移除。此操作不可撤销。',
        deleteNodeTitle: '删除节点？',
        deletePortMessage: '将删除端口 {port} 及其线路配置。此操作不可撤销。',
        deletePortTitle: '删除端口？',
        duplicateRouteForce: '强制添加新的',
        duplicateRouteMessage:
          '已存在完全相同的节点分享链接：{name}。可以复用旧节点，或强制添加一条新节点。',
        duplicateRouteReuse: '复用旧的',
        duplicateRouteTitle: '发现重复节点',
        removeRouteMessage: '将从端口 {port} 移除{kind}「{name}」。此操作不会删除对应对象。',
        removeRouteTitle: '移除线路？',
      },
      form: {
        listenAddress: '监听地址',
        name: '名称',
        nodeName: '节点名称',
        nodeUri: '节点 URI',
        outboundType: '端口类型',
        password: '密码',
        port: '端口',
        protocol: '协议',
        remark: '备注',
        server: '服务器',
        shareLink: '分享链接',
        strategy: '策略',
        groupName: '分组名称',
        groupStrategy: '分组策略',
        groupNodes: '包含节点',
        nodeGroup: '所属分组',
        tags: '标签',
        subscriptionName: '订阅名称',
        subscriptionUrl: '订阅 URL',
        subscriptionGroup: '所属分组',
        routeSource: '线路来源',
        existingNode: '已有节点',
        existingGroup: '已有分组',
        chainName: '串联名称',
        chainNodes: '串联成员',
        chainPreview: '链路顺序',
        searchNodes: '搜索节点',
        testUrl: '测试 URL',
        username: '用户名',
      },
      groupMeta: {
        groupCount: '{count} 个分组',
        noSelectedNodes: '暂未选择节点',
        nodeCount: '{count} 个节点',
        optionNodeCount: '{name}（{countLabel}）',
        selectedGroups: '{first}、{second} 等 {count} 个',
        summary: '{nodeCount} 个节点 · {groupCount} 个分组 · {builtins} 个内置出口',
        ungrouped: '默认',
      },
      groupFilters: {
        all: '全部',
        default: '默认',
        defaultDetail: '没有被任何分组引用的节点会出现在这里。',
        defaultStrategy: '虚拟分组',
        emptyNodes: '{name} 暂无节点。',
        hideEmpty: '隐藏 0 节点分组',
        summary: '分组视图',
        subscriptionLabel: '{name}(订阅)',
        virtual: '默认组',
      },
      groupSections: {
        cardCount: '{count} 张卡片',
        empty: '这个订阅下暂无分组卡片。',
        manual: '手动分组',
        noMatches: '没有匹配的分组卡片。',
        searchLabel: '搜索分组卡片',
        subscriptionCount: '{count} 个订阅',
        unknownSubscription: '未知订阅',
      },
      groupStrategy: {
        selector: '手动选择',
        'load-balance': '负载均衡',
        'least-latency': '最低延迟',
      },
      groupStrategyOverride: {
        inherit: '继承分组策略',
        overrideLabel: '覆盖：{strategy}',
      },
      groupType: {
        manual: '手动组',
        subscription: '订阅组',
      },
      hero: {
        lead: '先添加端口，再在端口卡片内添加线路。',
        title: '代理转发面板',
      },
      messages: {
        backendRefreshed: '已从后端重新加载。',
        demoReset: '已恢复示例数据。',
        endpointCopied: '已复制 {endpoint}',
        importEmpty: '请先粘贴分享链接。',
        imported: '已导入 {count} 条节点。',
        importedWithGroups: '已导入 {nodes} 个节点、{groups} 个分组，跳过 {skipped} 项。',
        groupAdded: '已新增分组 {name}。',
        groupUpdated: '已更新分组 {name}。',
        chainNodeAdded: '已新增串联节点 {name}。',
        chainNodesRequired: '请至少选择两个节点或节点组。',
        groupNameRequired: '请填写分组名称。',
        loadingBackend: '正在从后端加载代理配置…',
        loadingNodes: '正在加载节点…',
        mappingNotFound: '端口映射不存在，请刷新后重试。',
        nodeAdded: '已新增 {name}。',
        nodeNotFound: '节点不存在，请刷新后重试。',
        nodePortRequired: '请填写节点端口。',
        nodeServerRequired: '请填写节点服务器。',
        nodeUpdated: '已更新 {name}。',
        nodeUriCopied: '已复制 {name} 的 URI。',
        nodeUriUnavailable: '当前节点无法导出 URI。',
        requestFailed: '请求后端失败，请确认服务已启动。',
        routeNodeRequired: '请填写节点 URI。',
        routeExistingRequired: '请选择已有节点。',
        routeGroupRequired: '请选择已有分组。',
        routeSwitched: '已切换完成。已建立的连接不会被强制断开，新进入的连接会切换为新节点。',
        runtimeError: '运行时异常：{message}',
        runtimeFailureDetail: '{port}（{reason}）',
        runtimeFailureReason: '失败原因：{reason}',
        runtimeFailureSeparator: '；',
        runtimeFailureUnknown: '未知原因',
        runtimeFailureUnknownPort: '未知端口',
        runtimeFailures: '{count} 个端口启动失败：{details}',
        availableNodes: '当前可用节点 {available}/{total}。',
        runtimeExcludedNodes: '已自动排除 {count} 个不可用节点。',
        runtimeRunning: '已连接后端，代理运行中（{count} 个入口）。',
        runtimeStopped: '已连接后端，代理运行时未启动。',
        savingBackend: '正在保存到后端…',
        subscriptionSynced: '订阅已同步。',
        subscriptionUrlRequired: '请填写订阅 URL。',
      },
      importPreview: {
        action: {
          fail: '失败',
          import: '导入',
          skip: '跳过',
          update: '更新',
        },
        confirmImport: '确认导入',
        confirmSubscription: '确认添加并同步',
        previewSubscription: '预览订阅',
        summary: '共 {total} 项，导入 {imports} 项，更新 {updates} 项，跳过 {skipped} 项',
        title: '导入预览',
        type: {
          builtin: '内置',
          failure: '失败',
          group: '分组',
          node: '节点',
        },
      },
      nodeMeta: {
        chainCount: '{count} 跳串联',
        chainEmpty: '请选择两个或更多节点',
        passwordConfigured: '密码：已配置',
        username: '用户名：{value}',
      },
      nodeCreate: {
        batchImport: '批量导入',
        chainNode: '串联节点',
        uriNode: 'URI 节点',
      },
      nodeHealth: {
        blacklisted: '已排除',
        blacklistedPendingProbe: '已排除，等待健康探测恢复',
        blacklistedRecoveringSoon: '已排除，即将恢复',
        blacklistedRemaining: '已排除，{duration}后恢复',
        blacklistedUntilTime: '恢复时间：{time}',
        blacklistedWithReason: '已排除：{reason}',
        checkedAt: '最近探测：{time}',
        durationDays: '{count} 天',
        durationDaysHours: '{days} 天 {hours} 小时',
        durationHours: '{count} 小时',
        durationHoursMinutes: '{hours} 小时 {minutes} 分钟',
        durationMinutes: '{count} 分钟',
        failedShort: '失败',
        failure: '失败数',
        latency: '延迟',
        lastError: '失败原因：{reason}',
        probing: '测速中',
        probeStartedAt: '测速开始：{time}',
        summary: '延迟 {latency}，成功 {success}，失败 {failure}',
        success: '成功数',
        unavailable: '不可用',
      },
      nodeGroupHealth: {
        autoProbeDisabled: '当前分组不自动测速',
        autoProbeEnabled: '当前分组会自动测速',
        autoProbeRunning: '当前分组测速中',
        available: '可用 {count}',
        availableRatio: '可用 {available}/{total}',
        availableTitle: '可用节点',
        fastest: '最快 {latency}',
        fastestTitle: '最快延迟',
        needsProbe: '待测速 {count}',
        probing: '测速中 {count}',
        total: '共 {count}',
        totalTitle: '节点总数',
        unavailable: '不可用 {count}',
      },
      notice: '大号加号添加端口，小号加号把线路加入该端口。',
      outbound: {
        http: 'HTTP',
        mixed: 'HTTP+SOCKS5 混合',
        socks5: 'SOCKS5',
      },
      placeholders: {
        groupSearch: '搜索分组名称、策略或过滤规则',
        nodeName: '例如：香港节点',
        nodeUri: 'vless://...、hy2://... 或 ss://...',
        nodeSearch: '按节点名称搜索',
        testUrl: 'https://www.gstatic.com/generate_204',
        chainName: '例如：A 访问 B',
        tags: '多个标签用逗号分隔',
        shareLinks: 'vless://...\nhy2://...\nss://...\ntuic://...',
      },
      portState: {
        closed: '已关闭',
        failed: '启动失败',
        notRunning: '未运行',
        running: '运行中',
      },
      protocol: {
        http: 'HTTP',
        chain: '串联',
        hysteria: 'Hysteria',
        hysteria2: 'Hysteria2',
        socks5: 'SOCKS5',
        shadowsocks: 'Shadowsocks',
        ssh: 'SSH',
        trojan: 'Trojan',
        tuic: 'TUIC',
        unknown: '未知',
        vless: 'VLESS',
        vmess: 'VMess',
      },
      sections: {
        addPortHint: '新建一个本地端口入口',
        importLead: '导入后可回到端口卡片内绑定线路。',
        importTitle: '导入节点',
        mappingsLead: '每个卡片是一个本地端口，卡片内部是对接线路。',
        mappingsTitle: '本地端口',
        nodesLead: '先按分组筛选，再查看具体节点，避免上百条线路淹没有效信息。',
        nodesTitle: '节点',
        chainLead: '创建一个虚拟节点，按选择顺序通过前一个节点访问后一个节点。',
        chainTitle: '节点串联',
        groupsLead: '手动分组可维护成员，订阅组由同步自动覆盖。',
        groupsTitle: '节点分组',
        subscriptionsLead: '订阅只管理来源地址和所属分组，同步后节点会归入该分组。',
        subscriptionsTitle: '订阅管理',
        ungroupedNodesTitle: '默认',
      },
      status: {
        enabledPorts: '启用端口',
        groupCount: '分组总量',
        lastSaved: '最后保存',
        mappingCount: '端口卡片',
        nodeCount: '节点总量',
      },
      strategy: {
        failover: '故障转移',
        leastLatency: '最低延迟',
        loadBalance: '负载均衡',
        manual: '手动选择',
      },
      tabs: {
        import: '导入',
        groups: '节点分组',
        groupsWithCount: '节点分组（{count}）',
        groupsCompactWithCount: '分组 ({count})',
        mappings: '本地端口',
        mappingsWithCount: '本地端口（{enabled}/{total}）',
        mappingsCompact: '端口',
        nodes: '节点',
        nodesWithCount: '节点（{count}）',
        nodesCompactWithCount: '节点 ({count})',
        subscriptions: '订阅',
      },
      test: {
        checkedAt: '测试时间',
        failed: '失败',
        latency: '延迟',
        node: '节点',
        nodeId: '节点 ID',
        nodeTag: '出站 Tag',
        result: '结果',
        retest: '重测',
        routePath: '跳转路径',
        running: '测试中',
        selectedNode: '选中节点：{node}',
        success: '成功',
        target: '目标',
        waiting: '等待测试',
      },
      routeSource: {
        group: '已有分组',
        node: '已有节点',
        uri: '新增节点',
      },
      routeKind: {
        group: '节点组',
        node: '节点',
      },
      subscription: {
        createGroup: '自动创建同名分组',
        empty: '暂无订阅。',
        noGroup: '未绑定分组',
        notSynced: '未同步',
        source: '订阅',
      },
    },
    about: {
      title: '关于页面',
    },
    settings: {
      counts: {
        groups: '{count} 个分组',
        mappings: '{count} 个端口',
        nodes: '{count} 个节点',
        subscriptions: '{count} 个订阅',
      },
      eyebrow: '备份与恢复',
      export: {
        button: '导出设置',
        lead: '下载当前节点、节点组、订阅和端口映射。',
        title: '导出全设置',
      },
      extraUiInfo: {
        default: '默认',
        lead: '控制桌面端顶部额外 UI 信息的显示；“默认”当前按隐藏处理。',
        off: '关闭',
        on: '开启',
        title: '显示额外 UI 信息',
      },
      formatLabel: '文件格式',
      formatValue: 'ProxyHub ZIP / JSON v1',
      import: {
        button: '覆盖导入',
        confirmOverwrite: '我确认导入后会覆盖当前代理配置。',
        lead: '上传导出的 ZIP 或 JSON 文件，完整替换当前代理配置。',
        selectFile: '选择 ZIP 或 JSON 备份文件',
        title: '导入全设置',
      },
      restartConfirm: {
        confirmButton: '确认重启',
        message: '服务会短暂断开，并按已保存的监听地址重新启动。',
        title: '重启服务？',
      },
      service: {
        addressLabel: '本地 IP',
        addressPlaceholder: '留空表示 0.0.0.0',
        lead: '修改 HTTP 面板监听地址和端口，保存后重启生效。',
        portLabel: '端口',
        restartButton: '重启服务',
        restartNo: '否',
        restartRequiredLabel: '需要重启',
        restartYes: '是',
        runningLabel: '当前运行',
        saveButton: '保存监听设置',
        savedLabel: '已保存',
        title: '服务监听',
      },
      messages: {
        confirmRequired: '请先确认覆盖当前代理配置。',
        exported: '设置文件已开始下载。',
        fileReady: '备份文件已读取，可以导入。',
        fileType: '请选择 .json 或 .zip 文件。',
        imported:
          '已导入 {nodes} 个节点、{groups} 个分组、{subscriptions} 个订阅、{mappings} 个端口。',
        importedWithWarning: '设置已导入，但运行时重载失败：{warning}',
        invalidBackup: '备份文件格式不正确。',
        invalidJsonShape: '备份文件必须是 JSON 对象。',
        invalidListenPort: '端口必须是 1 到 65535 之间的整数。',
        listenSaved: '服务监听配置已保存，重启后生效。',
        requestFailed: '请求后端失败，请确认服务已启动。',
        restarting: '服务正在重启，请稍候…',
        restartRequested: '服务正在重启。',
        restartTimeout: '服务已请求重启，但还无法访问。请稍后打开 {url}',
        selectFile: '请先选择备份文件。',
        zipFileReady: 'ZIP 备份文件已选择，导入时将由后端校验。',
      },
      scopeLabel: '包含范围',
      scopeValue: '代理配置，不含账号和系统配置',
      title: '设置',
    },
    login: {
      errors: {
        failed: '登录失败，请检查用户名和密码。',
      },
      form: {
        password: '密码',
        submit: '登录',
        username: '用户名',
      },
      messages: {
        success: '登录成功，正在返回面板。',
      },
      placeholders: {
        password: '请输入密码',
        username: '请输入用户名',
      },
      title: '登录 ProxyHub',
    },
    register: {
      errors: {
        failed: '注册失败，请重试。',
        usernameTaken: '用户名已被占用。',
      },
      form: {
        loginLink: '已有账号？登录',
        nickname: '昵称',
        password: '密码',
        submit: '注册',
        username: '用户名',
      },
      link: '没有账号？点击注册',
      messages: {
        success: '注册成功，正在返回面板。',
      },
      placeholders: {
        nickname: '输入昵称',
        password: '请输入密码',
        username: '请输入用户名',
      },
      title: '注册 ProxyHub',
    },
    state: {
      node: {
        unnamed: '未命名节点',
        unsupportedRemark: '分享链接暂未完整解析，请手动补充服务器信息。',
        vmessDefault: 'VMess 节点',
      },
      seed: {
        mapping1Remark: '给局域网设备使用的默认入口',
        mapping2Remark: '混合端口备用入口',
        node1Name: '线路 1 - VLESS H2',
        node1Remark: '主线路',
        node2Name: '线路 2 - Trojan',
        node2Remark: '备用线路',
        node3Name: '线路 3 - SOCKS5',
        node3Remark: '内网跳板',
      },
    },
  },
  'en-US': {
    app: {
      name: 'ProxyHub',
      pageTitles: {
        about: 'About',
        groups: 'Node groups',
        login: 'Sign in',
        mappings: 'Local ports',
        nodes: 'Nodes',
        settings: 'Settings',
      },
    },
    common: {
      add: 'Add',
      addNode: 'Add node',
      addChainNode: 'Add chain node',
      addGroup: 'Add group',
      addPort: 'Add port',
      addRoute: 'Add route',
      addSubscription: 'Add subscription',
      authConfigured: 'Auth',
      cancel: 'Cancel',
      close: 'Close',
      copiedEndpoint: 'Endpoint copied',
      copyEndpoint: 'Copy endpoint',
      currentRoute: 'Current',
      deleteNode: 'Delete node',
      deleteGroup: 'Delete group',
      deletePort: 'Delete port',
      deleteSubscription: 'Delete subscription',
      copiedNodeUri: 'URI copied',
      editGroup: 'Edit group',
      editNode: 'Edit node',
      editPort: 'Edit port',
      exportNodeUri: 'Export URI',
      goHome: 'Back to panel',
      goLogin: 'Sign in',
      importLinks: 'Import links',
      language: 'Language',
      languageChinese: '简体中文',
      languageEnglish: 'English',
      languageSystem: 'Follow system',
      noAccount: 'No account configured',
      noAuth: 'No Auth',
      noRemark: 'No remark',
      notSaved: 'Not saved yet',
      optional: 'Optional',
      refreshStatus: 'Refresh status',
      removeRoute: 'Remove route',
      restoreDemo: 'Refresh data',
      save: 'Save',
      setActiveRoute: 'Set current',
      settings: 'Settings',
      updateBadge: 'Update',
      updateTitle: 'Open the release page for {version}',
      npmUpdateTitle: 'Open the release page for {version}. If installed via npm, run {command}',
      syncSubscription: 'Sync subscription',
      test: 'Test',
    },
    home: {
      actions: {
        loadMore: 'Load more',
      },
      aria: {
        activePorts: 'Port runtime status',
        addNodeMenu: 'Choose node type',
        closeDialog: 'Close',
        copyEndpoint: 'Copy endpoint',
        deleteNode: 'Delete node',
        deletePort: 'Delete port',
        disablePort: 'Disable port',
        editPort: 'Edit port',
        enablePort: 'Enable port',
        moreActions: 'More actions',
        removeFromPort: 'Remove from port',
        removeRoute: 'Remove route',
        nodeGroupFilters: 'Node group filters',
        statusOverview: 'Status overview',
        tabs: 'Page navigation',
      },
      dialogs: {
        addNodeLead: 'Leave the name empty to infer it from the URI.',
        addNodeTitle: 'Add node',
        addChainNodeTitle: 'Add chain node',
        addGroupLead: 'Create a manual group that can be used directly in port routes.',
        editGroupLead:
          'Adjust the strategy and members of a manual group. Ports using it refresh with the runtime.',
        editGroupTitle: 'Edit group',
        importNodeLead: 'Paste multiple share links, preview the changes, then confirm import.',
        importNodeTitle: 'Batch import nodes',
        editNodeTitle: 'Edit node',
        addPortTitle: 'Add port',
        editPortTitle: 'Edit port',
        mappingLead: 'Username and password are empty by default.',
        testMappingTitle: 'Test port',
        testNodeTitle: 'Test node',
      },
      confirm: {
        deleteGroupMessage:
          'Group "{name}" will be deleted and removed from port routes and nodes. This cannot be undone.',
        deleteGroupTitle: 'Delete group?',
        deleteNodeMessage:
          'Node "{name}" will be deleted and removed from port routes and node groups. This cannot be undone.',
        deleteNodeTitle: 'Delete node?',
        deletePortMessage:
          'Port {port} and its route configuration will be deleted. This cannot be undone.',
        deletePortTitle: 'Delete port?',
        duplicateRouteForce: 'Force add new',
        duplicateRouteMessage:
          'An identical node share link already exists: {name}. Reuse the existing node or force-add a new one.',
        duplicateRouteReuse: 'Reuse existing',
        duplicateRouteTitle: 'Duplicate node found',
        removeRouteMessage:
          'Remove {kind} "{name}" from port {port}. The referenced object will not be deleted.',
        removeRouteTitle: 'Remove route?',
      },
      form: {
        listenAddress: 'Listen address',
        name: 'Name',
        nodeName: 'Node name',
        nodeUri: 'Node URI',
        outboundType: 'Port type',
        password: 'Password',
        port: 'Port',
        protocol: 'Protocol',
        remark: 'Remark',
        server: 'Server',
        shareLink: 'Share links',
        strategy: 'Strategy',
        groupName: 'Group name',
        groupStrategy: 'Group strategy',
        groupNodes: 'Group nodes',
        nodeGroup: 'Node group',
        tags: 'Tags',
        subscriptionName: 'Subscription name',
        subscriptionUrl: 'Subscription URL',
        subscriptionGroup: 'Group',
        routeSource: 'Route source',
        existingNode: 'Existing node',
        existingGroup: 'Existing group',
        chainName: 'Chain name',
        chainNodes: 'Chain members',
        chainPreview: 'Route order',
        searchNodes: 'Search nodes',
        testUrl: 'Test URL',
        username: 'Username',
      },
      groupMeta: {
        groupCount: '{count} group(s)',
        noSelectedNodes: 'No nodes selected',
        nodeCount: '{count} node(s)',
        optionNodeCount: '{name} ({countLabel})',
        selectedGroups: '{first}, {second}, +{count}',
        summary: '{nodeCount} node(s) · {groupCount} group(s) · {builtins} built-in',
        ungrouped: 'Default',
      },
      groupFilters: {
        all: 'All',
        default: 'Default',
        defaultDetail: 'Nodes not referenced by any group appear here.',
        defaultStrategy: 'Virtual group',
        emptyNodes: '{name} has no nodes.',
        hideEmpty: 'Hide empty groups',
        summary: 'Group view',
        subscriptionLabel: '{name}(subscription)',
        virtual: 'Default',
      },
      groupSections: {
        cardCount: '{count} card(s)',
        empty: 'No group cards under this subscription.',
        manual: 'Manual groups',
        noMatches: 'No matching group cards.',
        searchLabel: 'Search group cards',
        subscriptionCount: '{count} subscription(s)',
        unknownSubscription: 'Unknown subscription',
      },
      groupStrategy: {
        selector: 'Manual select',
        'load-balance': 'Load balance',
        'least-latency': 'Least latency',
      },
      groupStrategyOverride: {
        inherit: 'Inherit group strategy',
        overrideLabel: 'Override: {strategy}',
      },
      groupType: {
        manual: 'Manual',
        subscription: 'Subscription',
      },
      hero: {
        lead: 'Add a port first, then attach routes inside the port card.',
        title: 'Proxy forwarding panel',
      },
      messages: {
        backendRefreshed: 'Reloaded from backend.',
        demoReset: 'Demo data restored.',
        endpointCopied: 'Copied {endpoint}',
        importEmpty: 'Paste a share link first.',
        imported: 'Imported {count} nodes.',
        importedWithGroups: 'Imported {nodes} node(s), {groups} group(s), skipped {skipped}.',
        groupAdded: 'Added group {name}.',
        groupUpdated: 'Updated group {name}.',
        chainNodeAdded: 'Added chain node {name}.',
        chainNodesRequired: 'Select at least two nodes or groups.',
        groupNameRequired: 'Enter a group name.',
        loadingBackend: 'Loading proxy config from backend...',
        loadingNodes: 'Loading nodes...',
        mappingNotFound: 'Port mapping not found. Refresh and try again.',
        nodeAdded: 'Added {name}.',
        nodeNotFound: 'Node not found. Refresh and try again.',
        nodePortRequired: 'Enter the node port.',
        nodeServerRequired: 'Enter the node server.',
        nodeUpdated: 'Updated {name}.',
        nodeUriCopied: 'Copied URI for {name}.',
        nodeUriUnavailable: 'This node cannot be exported as a URI.',
        requestFailed: 'Backend request failed. Make sure the service is running.',
        routeNodeRequired: 'Enter a node URI.',
        routeExistingRequired: 'Select an existing node.',
        routeGroupRequired: 'Select an existing group.',
        routeSwitched:
          'Route switch completed. Existing connections will not be forcibly disconnected; new connections will use the new node.',
        runtimeError: 'Runtime error: {message}',
        runtimeFailureDetail: '{port} ({reason})',
        runtimeFailureReason: 'Reason: {reason}',
        runtimeFailureSeparator: '; ',
        runtimeFailureUnknown: 'Unknown reason',
        runtimeFailureUnknownPort: 'Unknown port',
        runtimeFailures: '{count} port(s) failed to start: {details}',
        availableNodes: 'Available nodes: {available}/{total}.',
        runtimeExcludedNodes: '{count} unusable node(s) excluded automatically.',
        runtimeRunning: 'Backend connected, proxy runtime is running ({count} inbounds).',
        runtimeStopped: 'Backend connected, proxy runtime is stopped.',
        savingBackend: 'Saving to backend...',
        subscriptionSynced: 'Subscription synced.',
        subscriptionUrlRequired: 'Enter a subscription URL.',
      },
      importPreview: {
        action: {
          fail: 'Failed',
          import: 'Import',
          skip: 'Skip',
          update: 'Update',
        },
        confirmImport: 'Confirm import',
        confirmSubscription: 'Confirm add and sync',
        previewSubscription: 'Preview subscription',
        summary: '{total} item(s), {imports} import, {updates} update, {skipped} skipped',
        title: 'Import preview',
        type: {
          builtin: 'Builtin',
          failure: 'Failure',
          group: 'Group',
          node: 'Node',
        },
      },
      nodeMeta: {
        chainCount: '{count} hop chain',
        chainEmpty: 'Select two or more nodes',
        passwordConfigured: 'Password: configured',
        username: 'Username: {value}',
      },
      nodeCreate: {
        batchImport: 'Batch import',
        chainNode: 'Chain node',
        uriNode: 'URI node',
      },
      nodeHealth: {
        blacklisted: 'Excluded',
        blacklistedPendingProbe: 'Excluded, waiting for health probe recovery',
        blacklistedRecoveringSoon: 'Excluded, recovering soon',
        blacklistedRemaining: 'Excluded, recovers in {duration}',
        blacklistedUntilTime: 'Recovery time: {time}',
        blacklistedWithReason: 'Excluded: {reason}',
        checkedAt: 'Last probe: {time}',
        durationDays: '{count}d',
        durationDaysHours: '{days}d {hours}h',
        durationHours: '{count}h',
        durationHoursMinutes: '{hours}h {minutes}m',
        durationMinutes: '{count}m',
        failedShort: 'Failed',
        failure: 'Failures',
        latency: 'Latency',
        lastError: 'Reason: {reason}',
        probing: 'Probing',
        probeStartedAt: 'Probe started: {time}',
        summary: 'Latency {latency}, successes {success}, failures {failure}',
        success: 'Successes',
        unavailable: 'Unavailable',
      },
      nodeGroupHealth: {
        autoProbeDisabled: 'No auto probe for this group',
        autoProbeEnabled: 'Auto probe enabled for this group',
        autoProbeRunning: 'This group is probing',
        available: 'Available {count}',
        availableRatio: '{available}/{total}',
        availableTitle: 'Available nodes',
        fastest: 'Fastest {latency}',
        fastestTitle: 'Fastest latency',
        needsProbe: 'Needs probe {count}',
        probing: 'Probing {count}',
        total: 'Total {count}',
        totalTitle: 'Total nodes',
        unavailable: 'Unavailable {count}',
      },
      notice: 'Use the large plus to add a port and the small plus to attach a route.',
      outbound: {
        http: 'HTTP',
        mixed: 'HTTP+SOCKS5 mixed',
        socks5: 'SOCKS5',
      },
      placeholders: {
        groupSearch: 'Search group name, strategy, or filter',
        nodeName: 'e.g. Hong Kong node',
        nodeUri: 'vless://..., hy2://..., or ss://...',
        nodeSearch: 'Search by node name',
        testUrl: 'https://www.gstatic.com/generate_204',
        chainName: 'e.g. A via B',
        tags: 'Separate tags with commas',
        shareLinks: 'vless://...\nhy2://...\nss://...\ntuic://...',
      },
      portState: {
        closed: 'Closed',
        failed: 'Start failed',
        notRunning: 'Not running',
        running: 'Running',
      },
      protocol: {
        http: 'HTTP',
        chain: 'Chain',
        hysteria: 'Hysteria',
        hysteria2: 'Hysteria2',
        socks5: 'SOCKS5',
        shadowsocks: 'Shadowsocks',
        ssh: 'SSH',
        trojan: 'Trojan',
        tuic: 'TUIC',
        unknown: 'Unknown',
        vless: 'VLESS',
        vmess: 'VMess',
      },
      sections: {
        addPortHint: 'Create a local port entry',
        importLead: 'After import, return to a port card to bind routes.',
        importTitle: 'Import nodes',
        mappingsLead: 'Each card is a local port, with routes inside the card.',
        mappingsTitle: 'Local ports',
        nodesLead: 'Filter by group first, then inspect nodes so large libraries stay readable.',
        nodesTitle: 'Nodes',
        chainLead: 'Create a virtual node that reaches each later node through the previous node.',
        chainTitle: 'Node chaining',
        groupsLead: 'Manual groups are editable; subscription groups are replaced on sync.',
        groupsTitle: 'Groups',
        subscriptionsLead:
          'Subscriptions manage source URLs and target groups; synced nodes belong to that group.',
        subscriptionsTitle: 'Subscriptions',
        ungroupedNodesTitle: 'Default',
      },
      status: {
        enabledPorts: 'Enabled ports',
        groupCount: 'Groups',
        lastSaved: 'Last saved',
        mappingCount: 'Port cards',
        nodeCount: 'Nodes',
      },
      strategy: {
        failover: 'Failover',
        leastLatency: 'Least latency',
        loadBalance: 'Load balance',
        manual: 'Manual',
      },
      tabs: {
        import: 'Import',
        groups: 'Node groups',
        groupsWithCount: 'Node groups ({count})',
        groupsCompactWithCount: 'Groups ({count})',
        mappings: 'Local ports',
        mappingsWithCount: 'Local ports ({enabled}/{total})',
        mappingsCompact: 'Ports',
        nodes: 'Nodes',
        nodesWithCount: 'Nodes ({count})',
        nodesCompactWithCount: 'Nodes ({count})',
        subscriptions: 'Subscriptions',
      },
      test: {
        checkedAt: 'Checked at',
        failed: 'Failed',
        latency: 'Latency',
        node: 'Node',
        nodeId: 'Node ID',
        nodeTag: 'Outbound tag',
        result: 'Result',
        retest: 'Retest',
        routePath: 'Route path',
        running: 'Testing',
        selectedNode: 'Selected node: {node}',
        success: 'Success',
        target: 'Target',
        waiting: 'Waiting',
      },
      routeSource: {
        group: 'Existing group',
        node: 'Existing node',
        uri: 'New share link',
      },
      routeKind: {
        group: 'node group',
        node: 'node',
      },
      subscription: {
        createGroup: 'Create a matching group',
        empty: 'No subscriptions yet.',
        noGroup: 'No group',
        notSynced: 'Not synced',
        source: 'Subscription',
      },
    },
    about: {
      title: 'About',
    },
    settings: {
      counts: {
        groups: '{count} groups',
        mappings: '{count} ports',
        nodes: '{count} nodes',
        subscriptions: '{count} subscriptions',
      },
      eyebrow: 'Backup and restore',
      export: {
        button: 'Export settings',
        lead: 'Download current nodes, groups, subscriptions, and port mappings.',
        title: 'Export all settings',
      },
      extraUiInfo: {
        default: 'Default',
        lead: 'Control the desktop-only extra UI info at the top; "Default" currently behaves like hidden.',
        off: 'Off',
        on: 'On',
        title: 'Show extra UI info',
      },
      formatLabel: 'File format',
      formatValue: 'ProxyHub ZIP / JSON v1',
      import: {
        button: 'Overwrite import',
        confirmOverwrite: 'I understand this will overwrite the current proxy configuration.',
        lead: 'Upload an exported ZIP or JSON file to replace the current proxy configuration.',
        selectFile: 'Choose ZIP or JSON backup file',
        title: 'Import all settings',
      },
      restartConfirm: {
        confirmButton: 'Restart',
        message: 'The service will disconnect briefly and start again with the saved listen address.',
        title: 'Restart service?',
      },
      service: {
        addressLabel: 'Local IP',
        addressPlaceholder: 'Blank means 0.0.0.0',
        lead: 'Change the HTTP panel listen address and port. Restart to apply saved changes.',
        portLabel: 'Port',
        restartButton: 'Restart service',
        restartNo: 'No',
        restartRequiredLabel: 'Restart needed',
        restartYes: 'Yes',
        runningLabel: 'Running',
        saveButton: 'Save listen settings',
        savedLabel: 'Saved',
        title: 'Service listen',
      },
      messages: {
        confirmRequired: 'Confirm overwrite before importing.',
        exported: 'The settings file download has started.',
        fileReady: 'Backup file is ready to import.',
        fileType: 'Choose a .json or .zip file.',
        imported:
          'Imported {nodes} nodes, {groups} groups, {subscriptions} subscriptions, and {mappings} ports.',
        importedWithWarning: 'Settings imported, but runtime reload failed: {warning}',
        invalidBackup: 'The backup file format is invalid.',
        invalidJsonShape: 'The backup file must be a JSON object.',
        invalidListenPort: 'Port must be an integer from 1 to 65535.',
        listenSaved: 'Service listen settings saved. Restart to apply them.',
        requestFailed: 'Backend request failed. Check that the service is running.',
        restarting: 'Restarting service...',
        restartRequested: 'Service restart requested.',
        restartTimeout: 'Restart was requested, but the service is not reachable yet. Open {url} shortly.',
        selectFile: 'Choose a backup file first.',
        zipFileReady: 'ZIP backup file is selected and will be validated by the backend on import.',
      },
      scopeLabel: 'Includes',
      scopeValue: 'Proxy config, excluding accounts and system config',
      title: 'Settings',
    },
    login: {
      errors: {
        failed: 'Sign-in failed. Check your username and password.',
      },
      form: {
        password: 'Password',
        submit: 'Sign in',
        username: 'Username',
      },
      messages: {
        success: 'Signed in. Returning to the panel.',
      },
      placeholders: {
        password: 'Enter password',
        username: 'Enter username',
      },
      title: 'Sign in to ProxyHub',
    },
    register: {
      errors: {
        failed: 'Registration failed. Please try again.',
        usernameTaken: 'Username is already taken.',
      },
      form: {
        loginLink: 'Already have an account? Sign in',
        nickname: 'Nickname',
        password: 'Password',
        submit: 'Register',
        username: 'Username',
      },
      link: "Don't have an account? Register",
      messages: {
        success: 'Registered. Returning to the panel.',
      },
      placeholders: {
        nickname: 'Enter nickname',
        password: 'Enter password',
        username: 'Enter username',
      },
      title: 'Register ProxyHub',
    },
    state: {
      node: {
        unnamed: 'Untitled node',
        unsupportedRemark:
          'The share link was not fully parsed. Fill in the server details manually.',
        vmessDefault: 'VMess node',
      },
      seed: {
        mapping1Remark: 'Default entry for LAN devices',
        mapping2Remark: 'Backup mixed-port entry',
        node1Name: 'Route 1 - VLESS H2',
        node1Remark: 'Primary route',
        node2Name: 'Route 2 - Trojan',
        node2Remark: 'Backup route',
        node3Name: 'Route 3 - SOCKS5',
        node3Remark: 'LAN jump host',
      },
    },
  },
} as const;
